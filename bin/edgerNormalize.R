...
write.table(df.rpkm, file=paste0(args[2], "_featureCounts_RPKM.tsv"), row.names=T, quote=F, sep="\t")
write.table(df.cpm, file=paste0(args[2], "_featureCounts_CPM.tsv"), row.names=T, quote=F, sep="\t")
write.table(df.tpm, file=paste0(args[2], "_featureCounts_TPM.tsv"), row.names=T, quote=F, sep="\t")
